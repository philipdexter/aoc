USING: math kernel io prettyprint locals sequences arrays assocs sets vectors ;
IN: 71

: entry-weight ( entry -- weight ) first ;
: entry-to ( entry -- to ) second ;
: into-entry ( weight to -- entry ) 2array ;
...
:: part1 ( -- )
    entries [ drop ] { } assoc>map >vector :> nodes
    entries [ swap drop entry-to ] { } assoc>map
    [ [| el | el nodes delete ] each ] each
    nodes . ;
...
: total-of ( str -- x )
    entries at dup entry-weight swap entry-to
    [ total-of ] map sum
    + ;
...
:: process ( root -- )
    root entries at entry-to [ total-of ] map :> sub-totals
    sub-totals all-same? not [ root . root entries at entry-weight . root entries at entry-to . sub-totals . ] [ ] if
    root entries at entry-to [ process ] each ;
