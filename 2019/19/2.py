...
CACHE = {}
def try_at(next_x, next_y):
  global to_give
  global give_index
  global agg

  to_give = [z for x in range(next_x, next_x + 100) for y in range(next_y, next_y + 100) for z in (x, y)]
  give_index = 0
  agg = 0
  last_agg = agg
  while len(to_give) > give_index:
    reset()
    ci = (to_give[give_index], to_give[give_index+1])
    if ci in CACHE:
      agg += CACHE[ci]
      give_index += 2
    else:
      go()
      CACHE[ci] = agg - last_agg
    last_agg = agg
  print('done round')
  print(f'found {agg}')
  print(f'should be {100*100}')
  print(f'{next_x * 10000 + next_y}')
  return agg
